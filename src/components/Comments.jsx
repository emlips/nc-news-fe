...
function Comments({ article }) {
  ...
  const loadComments = (num) => {
    ...
    getMoreComments(article.article_id, page)
  ...
  return (
    <>
      ...
      <button onClick={() => loadComments(-1)} disabled={commentPage === 1 ? true : false}>Previous Page</button>
      <button onClick={() => loadComments(1)} disabled={commentPage === totalPages ? true : false}>Next Page</button>
      <p>Viewing page {commentPage} of {totalPages}</p>
    </>
  );
}
